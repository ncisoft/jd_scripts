...
if (process.env.PLANT_BEAN_SHARECODES) {
  if (process.env.PLANT_BEAN_SHARECODES.indexOf('@') > -1) {
    ...
    let shareCodesList = process.env.PLANT_BEAN_SHARECODES.split('@');
    jdPlantBeanShareArr = shareCodesList
    for (let i = 0; i < jdPlantBeanShareArr.length; i++) {
      console.log("  [ " + i + "]" + jdPlantBeanShareArr[i])
    }
    console.log()
  }
} else {
...
async function doHelp() {
  const shareCodes=[...(jdPlantBeanShareArr || []), ...([])];
